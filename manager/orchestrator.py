...
class SubtitleOrchestrator:
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            logger.warning("Running in mock mode - messages will be logged but not queued")
    ...
    async def _declare_queues(self) -> None:
        ...
        await self.channel.declare_queue(
            self.download_queue_name,
            durable=True
        )
        
        # Declare translation queue
        await self.channel.declare_queue(
            self.translation_queue_name,
            durable=True
        )
        
        logger.info("Queues declared successfully")
    
    async def enqueue_download_task(self, request: SubtitleRequest, request_id: UUID) -> bool:
        """Enqueue a subtitle download task."""
        if not self.channel:
            logger.warning(f"Mock mode: Would enqueue download task for request {request_id}")
            logger.warning(f"Task data: {request.dict()}")
            return True
        ...
        try:
            ...
            message = Message(
                body=download_task.json().encode(),
    ...
    async def enqueue_translation_task(
        ...
        if not self.channel:
            logger.warning(f"Mock mode: Would enqueue translation task for request {request_id}")
            logger.warning(f"Task data: file={subtitle_file_path}, source={source_language}, target={target_language}")
            return True
        ...
        try:
            ...
            message = Message(
                body=translation_task.json().encode(),
                delivery_mode=aio_pika.DeliveryMode.PERSISTENT
            )
