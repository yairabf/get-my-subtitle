...
class TestRedisJobClientJobOperations:
    ...
    async def test_get_job_returns_none_when_job_not_found(
        self, fake_redis_job_client
    ):
...
class TestRedisJobClientStatusTracking:
    ...
    async def test_update_job_status_updates_fields(
        self, fake_redis_job_client, sample_subtitle_response, status, error_message, download_url
    ):
...
class TestRedisJobClientEventHistory:
    ...
    async def test_get_job_events_retrieves_event_history(
        ...
        events_to_record = [
            ...
            ("subtitle.translated", {"translated_path": "/path/to/translated.srt"}, "translator"),
        ]
...
class TestRedisJobClientTTLManagement:
    ...
    async def test_save_job_sets_ttl_for_completed_status(
        self, fake_redis_job_client
    ):
    ...
    async def test_save_job_sets_ttl_for_failed_status(
        self, fake_redis_job_client
    ):
    ...
    async def test_save_job_no_ttl_for_active_status(
        self, fake_redis_job_client
    ):
...
class TestRedisJobClientErrorHandling:
    ...
    async def test_get_job_returns_none_when_not_connected(
        self, sample_job_id
    ):
    ...
    async def test_update_job_status_returns_false_when_not_connected(
        ...
        result = await client.update_job_status(
            sample_job_id, SubtitleStatus.DONE
        )
    ...
    async def test_record_event_returns_false_when_not_connected(
        self, sample_job_id
    ):
