...
from common.subtitle_parser import SubtitleSegment, SRTParser
...
class TestSubtitleTranslator:
    ...
    async def test_translate_batch_with_api(self, translator_with_api_key):
        ...
        translator.client.chat.completions.create = AsyncMock(return_value=mock_response)
    ...
    def test_parse_translation_response(self, translator_without_api_key):
        ...
        response = "[1]\nHola\n\n[2]\nAdiós\n\n"
        translations = translator_without_api_key._parse_translation_response(response, 2)
        
        assert len(translations) == 2
        assert "Hola" in translations[0]
        assert "Adiós" in translations[1]
    
    def test_parse_translation_response_mismatched_count(self, translator_without_api_key):
