...
class SubtitleTranslator:
    ...
    def __init__(self):
        ...
        if settings.openai_api_key:
            ...
            logger.info(f"Initialized OpenAI async client with model: {settings.openai_model}")
        else:
            logger.warning("OpenAI API key not configured - translator will run in mock mode")
    ...
    async def translate_batch(
        ...
        """
        if not self.client:
            logger.warning("Mock mode: Returning original texts with [TRANSLATED] prefix")
...
async def process_translation_message(
    ...
    try:
        ...
        if not all([request_id_str, subtitle_file_path, source_language, target_language]):
...
"""
        ...
        for chunk_idx, chunk in enumerate(chunks, 1):
            logger.info(f"Translating chunk {chunk_idx}/{len(chunks)} ({len(chunk)} segments)")
